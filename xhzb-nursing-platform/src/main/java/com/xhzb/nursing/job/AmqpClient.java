...
public class AmqpClient implements ApplicationRunner {
    ...
    private void processMessage(Message message) {
        ...
        try {
            ...
            JSONObject entries = JSONUtil.parseObj(contentStr);
            JSONObject notifyData = entries.getJSONObject("notify_data");
            // 如果notify_data字段数据为空，直接返回
            if(ObjectUtil.isEmpty(entries)){
                return;
            }
            IotMsgNotifyData iotMsgNotifyData = BeanUtil.toBean(notifyData, IotMsgNotifyData.class);
            //如果属性上报为空，程序结束
            if(ObjectUtil.isEmpty(iotMsgNotifyData) || ObjectUtil.isEmpty(iotMsgNotifyData.getBody().getServices())){
                return;
            }
            //  交给service层处理逻辑
